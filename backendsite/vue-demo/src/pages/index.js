...
// 7.职能组新增下级职能组，点击新增时，接口判断返回提示（点击新增接口更新） today
// 8.编辑组织的时候，哪些属性可修改（需要翠翠给我）
// 3.点击删除，判断逻辑（！！是否有自组织启用接口！！）
// 4.点击编辑本组织（确定哪些能修改），点击保存（判断启用或者禁用组织逻辑），禁用后，右侧置灰（包括子元素）（！！是否有自组织启用接口！！）
// 5.职能单元新增职能组的额外逻辑：（新增职能单元组织中，是否已经有业务型组织，没有业务型组织提示“职能单元必须有一个职能类型为业务型”）（！！点击新增接口更新！！）
// 6.职能单元新增下级职能单元，点击新增，如果当前是业务类型，需要判断所在同层组织师是否已经有业务型，如果有的话提示不能添加，只能添加管理型（！！点击新增保存接口更新！！）
// 1.业务型职能单元，职能组，职能小组添加标记；拓展类型文字+hover效果；扩展类型+hover效果，选中效果
